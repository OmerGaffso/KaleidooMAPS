define([
  ...
  var ge = 32767,
    ...
    ye = new ae.Cartographic(),
    fe = new ae.Cartesian2(),
  ...
  function xe(e, t, r, n, i, o, a, s, d) {
    ...
    for (var m = e.length, g = 0; g < m; ++g) {
      var T = e[g],
        p = r[T],
        T = n[T];
      (ye.longitude = oe.CesiumMath.lerp(I, l, T.x)),
        (ye.latitude = oe.CesiumMath.lerp(h, u, T.y)),
        (ye.height = p - t);
      p = o.cartographicToCartesian(ye, Te);
      se.Matrix4.multiplyByPoint(a, p, p),
        ae.Cartesian3.minimumByComponent(p, s, s),
        ae.Cartesian3.maximumByComponent(p, d, d),
        (c = Math.min(c, ye.height));
    }
    return c;
  }
  function Me(e, t, r, n, i, o, a, s, d, c, u, h, l, I, m) {
    var g = ie.defined(a),
      ...
      E = d.east,
      y = d.west;
    E < y && (E += oe.CesiumMath.TWO_PI);
    for (var f = r.length, v = 0; v < f; ++v) {
      var N = r[v],
        w = i[N],
        x = o[N];
      (ye.longitude = oe.CesiumMath.lerp(y, E, x.x) + I),
        (ye.latitude = oe.CesiumMath.lerp(p, T, x.y) + m),
        (ye.height = w - c);
      var M,
        b,
        C = s.cartographicToCartesian(ye, Te);
      g &&
        ((M = 2 * N),
        (fe.x = a[M]),
        (fe.y = a[1 + M]),
        1 !== u &&
          ((w = de.AttributeCompression.octDecode(fe.x, fe.y, ve)),
          ...
          ae.Cartesian3.normalize(w, w),
          de.AttributeCompression.octEncode(w, fe))),
        n.hasWebMercatorT &&
          (b =
            (ue.WebMercatorProjection.geodeticLatitudeToMercatorAngle(
              ...
              h) *
            l),
        (t = n.encode(e, t, C, x, ye.height, fe, b));
    }
  }
  function be(e, t) {
    var r;
  ...
  return a(function (e, t) {
    var r,
      ...
      y = se.Matrix4.inverseTransformation(E, new se.Matrix4());
    s &&
    ...
    for (
      ...
      (ye.longitude = oe.CesiumMath.lerp(c, h, _)),
        (ye.latitude = oe.CesiumMath.lerp(u, l, H)),
        (ye.height = k),
        (P = Math.min(ye.longitude, P)),
        (W = Math.max(ye.longitude, W)),
        (D = Math.min(ye.latitude, D)),
        (B = Math.max(ye.latitude, B));
      V = I.cartographicToCartesian(ye);
        ...
        s &&
          (C[F] =
            (ue.WebMercatorProjection.geodeticLatitudeToMercatorAngle(
              ...
              r) *
            n),
        se.Matrix4.multiplyByPoint(y, V, Te),
    ...
    var j = g,
      j = Math.min(
        j,
        xe(e.westIndices, e.westSkirtHeight, M, x, d, I, y, S, A)
      );
    (j = Math.min(
      j,
      xe(e.southIndices, e.southSkirtHeight, M, x, d, I, y, S, A)
    )),
      (j = Math.min(
        j,
        xe(e.eastIndices, e.eastSkirtHeight, M, x, d, I, y, S, A)
      )),
      (j = Math.min(
        j,
        xe(e.northIndices, e.northSkirtHeight, M, x, d, I, y, S, A)
      ));
    for (
      var q,
        Q,
      ...
      w &&
        ...
        (fe.y = o[1 + K]),
        1 !== m &&
          ((q = de.AttributeCompression.octDecode(fe.x, fe.y, ve)),
          ...
          ae.Cartesian3.normalize(q, q),
          de.AttributeCompression.octEncode(q, fe))),
        (ee = Z.encode($, ee, b[te], x[te], M[te], fe, C[te]));
